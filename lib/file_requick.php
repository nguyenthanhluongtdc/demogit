...
function saveOrder() {
	...
	global $information, $phpmailer, $http;
	...
	// if($_SESSION['captcha_contact'] == md5($_POST['captcha_code'])) {
		...
		foreach ($_SESSION['cart'] as $id => $quantity) {
			...
			$r_product['title'] = '<a href="'.$http.$_SERVER['SERVER_NAME'].getURL($r_product['uri']).'" target="_blank">'.$r_product['title'].'</a>';
			if(is_array($_SESSION['cart_combo'][$id])) {
				$r_product['title'] = '<a href="'.$http.$_SERVER['SERVER_NAME'].getURL($r_product['uri']).'" target="_blank">'.$r_product['title'].'</a> (<font style="color:red;">'.number_format($price).$layout['product-item']['text-2']['value'].'</font> - <del class="text-muted">'.number_format($price_del).$layout['product-item']['text-2']['value'].'</del>)';
				foreach ($_SESSION['cart_combo'][$id] as $r_combo) {
					...
					$r_product['title'] .= '<font style="font-size:12px;"><br>+&nbsp;<a href="'.$http.$_SERVER['SERVER_NAME'].getURL($r_cp['uri']).'" target="_blank">'.$r_cp['title'].'</a> (<font style="color:red;">'.number_format($r_combo['price_sale']).$layout['product-item']['text-2']['value'].'</font> - <del class="text-muted">'.number_format($r_combo['price_origin']).$layout['product-item']['text-2']['value'].'</del>)</font>';
				}
			}
			if($price > 0) {
		...
		$condition = "";
		$data = array("name", "gender", "tel", "email", "address", "content", "detail", "create_date");
		if(@$_POST['delivery'] == "1") {
			$province_name = getItems(array("table" => "province", "id" => $_POST['pid']));
			$district_name = getItems(array("table" => "district", "id" => $_POST['did']));
			$_POST['address'] = sprintf("<span style='display: inline-block; min-width: 132px; font-weight: normal;'>Địa chỉ:</span> %s<br><span style='display: inline-block; min-width: 132px; font-weight: normal;'>Tỉnh - Thành phố:</span> %s<br><span style='display: inline-block; min-width: 132px; font-weight: normal;'>Quận - huyện:</span> %s", $_POST['address'], $province_name['type']." ".$province_name['name'], $district_name['type']." ".$district_name['name']);
			$_POST['store'] = "";
		}
		else {
		...
		if(@$_POST['staff'] != "")
			$_POST['content'] = "<div style='margin-bottom: 5px;'>( ".$_POST['staff']." )</div>".$_POST['content'];
		...
		foreach ($cart['product'] as $k_product => $r_product) {
			$r_product['color'] = "Mặc định";
			$r_c = getItems(array("table" => "option", "id" => $_SESSION['cart_color'][$r_product['id']]));
			if(is_array($r_c) && !empty($r_c))
				$r_product['color'] = $r_c['title'];
		$detail .= '<tr>
					<td style="text-align: center; padding: 8px;">'.($k_product + 1).'</td>
					<td style="text-align: left; padding: 8px;"><b>'.$r_product['title'].'</b></td>
					<td style="text-align: center; padding: 8px;"><b style="color: red;">'.$r_product['price'].'</b> <b style="color: #777;">'.$r_product['price_del'].'</b></td>
					<td style="text-align: center; padding: 8px; color: blue; font-weight: bold;">'.$r_product['quantity'].'</td>
					<td style="text-align: center; padding: 8px;"><b style="color: red;">'.$r_product['total_price'].'</b> <b style="color: #777;">'.$r_product['total_price_del'].'</b></td>
					<td style="text-align: center; padding: 8px;"><b>'.$_SESSION['cart_type'][$r_product['id_root']].'</b></td>
				</tr>';
		}
	  ...
	  foreach ($data_email as $k => $d)
	      $html = str_replace("#".$k."#", $d, $html);
	  	  $html = '<div style="margin-top: 20px; margin-bottom: 10px; font-weight: bold;">Thông tin đơn hàng</div><table class="table table-striped table-bordered" style="width: 100%;" border="1">
			<tbody>
				<tr>
					<td style="text-align: center; padding: 8px;"><strong>Họ tên</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Giới tính</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Điện thoại</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Email</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Địa chỉ</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Thành phố</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Quận/Huyện</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Ghi chú giao hàng</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>Ngày đặt hàng</strong></td>
				</tr>
				<tr>
					<td style="text-align: center; padding: 8px;"><strong>'.$_POST['name'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.$_POST['gender'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.$_POST['tel'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.$_POST['email'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.$_POST['address'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.$province_name[0]['name'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.$district_name[0]['name'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.$_POST['content'].'</strong></td>
					<td style="text-align: center; padding: 8px;"><strong>'.date('d/m/Y', time()).'</strong></td>
				</tr>
			</tbody>
	  </table><div style="margin-top: 20px; margin-bottom: 10px; font-weight: bold;">Chi tiết đơn hàng:</div>';
	  $html .= $_POST['detail'];
	  $sender = array(
	      "host" => $information['email_host'],
	      "port" => $information['email_port'],
