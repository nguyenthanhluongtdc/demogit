...
foreach ($_SESSION['cart'] as $id => $value_id) {
	
	foreach($value_id as $color => $value_color){

		foreach($value_color as $size => $quantity){
...
<script>
	...
	function cartReload() {
		...
		if($("#cart_load").length > 0)
			$("#cart_load").load(" #cart_load > #cart_load1");
	}
